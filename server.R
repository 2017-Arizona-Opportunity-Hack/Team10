...
attempts = read.csv("Attempts_Hackathon_5_Years_of_Data.csv")
missing = read.csv("Hackathon_Missing_Child_5_Years_of_Data.csv")
...
shinyServer(function(input, output, session) {
  ...
  myData = reactive({
    ...
    if (!is.null(input$zip)) {
      attemptsPoints = attemptsPoints[attemptsPoints$'Zip.Code' %in% input$zip,]
      missingPoints = missingPoints[missingPoints$'Zip.Code' %in% input$zip,]
    }
